import { useGetEmpresasResumen } from '../../../hooks/Catalogos/Empresas/useEmpresa' 
...
const EmpresasSettings = () => {  

    const { id } = useParams();

    const { getResumen, empresasResumen, isLoading } = useGetEmpresasResumen();
    ...
    useEffect(() => {
      if(empresasResumen.status === 200){
       console.log(empresasResumen.data);
      }
    },[empresasResumen]);
    ...
    return (
      <>
        ...
        <Row className="g-3">
          <Col lg={8}>
            <ProfileSettings mode = 'update' initialData = {empresasResumen.data}/>
            <ExperiencesSettings />
            <EducationSettings />
          </Col>
          <Col lg={4}>
