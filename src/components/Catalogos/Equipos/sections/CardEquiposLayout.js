...
export const CardEquiposLayout = ({ data, isSelectedItem, toggleSelectedItem }) => {
    console.log("data",data)
    return (
      <div className="d-flex flex-column gap-3">
        {data.slice(0, 12).map((equipo, index) => (
          <div
            key={index}
          ...
          >
            <div className="d-flex align-items-start align-items-sm-center">
              ...
              >
                <Form.Check.Input
                  type="checkbox"
                  checked={isSelectedItem(equipo.ID)}
                  onChange={() => toggleSelectedItem(equipo.ID)}
                />
              </Form.Check>
              <Link
              ...
              >
              <Avatar size="3xl" name={equipo.avatar.name} />
              </Link>
              <div className="ms-1 ms-sm-3">
                <p className="fw-semi-bold mb-3 mb-sm-2">
                  <Link to={`/catalogo/equipos/view-equipo/${equipo.ID}`}>{equipo.Nombre}</Link>
                </p>
                <Row className="align-items-center gx-0 gy-2">
                  ...
                  <Col xs="auto">
                    <h6 className="mb-0 text-500">{equipo.Integrantes}</h6>
                  </Col>
                </Row>
              </div>
            </div>
