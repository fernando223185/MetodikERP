...
import { useActDepartamentos } from "hooks/Catalogos/Departamentos/useDepartamentos";
...
function TableDepartamentos({departamentos, estatus, layout, setFilter, filter}) {
    ...
    const {actDepartamento, result:response, isLoading} = useActDepartamentos();

    useEffect(() => {
        if(departamentos && departamentos.status === 200 && departamentos.data.length > 0) {
            const transformedData = departamentos.data.map(u => ({
                estatus: (
                    <SubtleBadge pill
                    bg={classNames({
    ...
    const handleRowClick = (id) => {
        navigate(`/catalogo/view-departamento/${id}`);
    };
