...
function TableCustomers({ cte }) {
  ...
  useEffect(() => {
    if (cte && cte.status === 200 && cte.data.length > 0) 
    {
      const transformedData = cte.data.map(u => ({
        acciones: (
            <Link
              to={`/comercial/cte/reservaD/${u.ID}`}  
  ...
  useEffect(() => {
    if (response && response.status === 200) {
      ...
      {
        setTimeout(() => {
          navigate(`/comercial/cte/reservaD/${response.data[0].NID}`);
        }, 1000);
      }
    }
  },[response])
