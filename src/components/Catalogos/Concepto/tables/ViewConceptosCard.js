...
import { ConceptosCardLayout } from '../Profiles/ConceptosCardLayout';

const ViewConceptosCard = ({conceptos, estatus, layout, setFilter}) => {
    ...
    const conceptosIDs = conceptos.data.map(concepto => concepto.ID);
    const { selectedItems, isSelectedItem, toggleSelectedItem } = useBulkSelect(conceptosIDs);
    const [allConceptos] = useState(conceptos.data.slice(0, 21));
    const [primaryConceptos, setPrimaryConceptos] = useState(allConceptos);
    ...
    const {
        ...
        prevPage,
        goToPage
    } = usePagination(primaryConceptos, 7);
    ...
    return(
      ...
      <Col xxl={2} xl={3}>
            ...
            >
                ...
                <ConceptosFilterForm  estatus={estatus} setFilter={setFilter} />
            </Offcanvas>
        </Col>
    </Row>
  );
...
export default ViewConceptosCard;