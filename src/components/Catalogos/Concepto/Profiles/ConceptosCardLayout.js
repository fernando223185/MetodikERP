...
export const ConceptosCardLayout = ({data, isSelectedItem, toggleSelectedItem}) => {

    return (
    <div className='d-flex flex-column gap-3'>
            {data.slice(0,12).map((conceptos,index)=> (
                <div 
                key={index}
                ...
                >
                  <div className="d-flex align-items-start align-items-sm-center">
                    ...
                    >
                      <Form.Check.Input
                        type="checkbox"
                        checked={isSelectedItem(conceptos.id)}
                        onChange={() => toggleSelectedItem(conceptos.id)}
                      />
                    </Form.Check>
                    <Link
                        to={`/Catalogos/view-concepto/${conceptos.id}`}
                        className="d-none d-sm-block"
                    >
                      <Avatar size="3xl" name={conceptos.avatar.name} />
                    </Link>
                    <div className='ms-1 ms-sm-3'>
                        <p className='fw-semi-bold mb-3 mb-sm-2'>
                            <Link to={`/Catalogos/view-concepto/${conceptos.id}`}>{conceptos.Nombre}</Link>
                        </p>
                        <Row className="align-items-center gx-0 gy-2">
                            ...
                            <Col xs="auto">
                                <h6 className='mb-0 text-500'>{conceptos.FechaEmision}</h6>
                            </Col>
                        </Row>
                    </div>
                </div>
