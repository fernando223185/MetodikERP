...
export const VistaTarjetaLayoutPaqueteriaE = ({ data, isSelectedItem, toggleSelectedItem }) => {

  return (
    <div className='d-flex flex-column gap-3'>
            {data.slice(0,12).map((paqueteriaE,index)=> (
                <div 
                key={index}
                ...
                >
                  <div className="d-flex align-items-start align-items-sm-center">
                    ...
                    >
                      <Form.Check.Input
                        type="checkbox"
                        checked={isSelectedItem(paqueteriaE.id)}
                        onChange={() => toggleSelectedItem(paqueteriaE.id)}
                      />
                    </Form.Check>
                    <Link
                    ...
                    >
                      <Avatar size="3xl" name={paqueteriaE.avatar.name} />
                    </Link>
                    <div className='ms-1 ms-sm-3'>
                        <p className='fw-semi-bold mb-3 mb-sm-2'>
                            <Link to={`/Explorador/PaqueteriaEntrega`}>{paqueteriaE.folio}</Link>
                        </p>
                        <p className='fw-semi-bold mb-3 mb-sm-2'>
                            <Link to={`/Explorador/PaqueteriaEntrega`}>{paqueteriaE.avatar.name}</Link>
                        </p>
                        <Row className="align-items-center gx-0 gy-2">
                            ...
                            <Col xs="auto" className="lh-1 me-3">
                                {paqueteriaE.cantidad}
                            </Col>
                            <Col xs="auto">
                                <h6 className='mb-0 text-500'>{paqueteriaE.fecha}</h6>
                            </Col>
                        </Row>
                    </div>
                </div>
