...
import { useActEquipajeDetalle, useDelRowEquipajeD } from 'hooks/Comercial/Reserva/useReservaD'
...
function TablePaqueteriaD({ equipajeD, setUpdateList }) {

  const [result, setResult] = useState([]);
  const [editableRows, setEditableRows] = useState({});
  const { actPaqueteriaD, result: resultD, isLoading } = useActEquipajeDetalle();
  const { delRowEquipajeD, result: resultDel , isLoading: isLoadingDel } = useDelRowEquipajeD();
  ...
  const handleDeletedRow = async (id, RowID) => {
    try {
      await delRowEquipajeD({ id, RowID });      
  ...
  useEffect(() => {
    if (equipajeD) {
      const transformedData = equipajeD.map(u => ({
        acciones: (
          <>
