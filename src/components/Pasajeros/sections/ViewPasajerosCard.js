...
import AllPasajerosHeader from '../tables/AllPasajerosHeader';
import PasajerosFilterForm from './PasajerosFilterForm';
import { PasajerosCardLayout } from './PasajerosCardLayout';

const ViewPasajerosCard = ({pasajeros, estatus, layout, setFilter}) => {
    ...
    const pasajeroIDs = pasajeros.data.map(pasajero => pasajero.id);
    const { selectedItems, isSelectedItem, toggleSelectedItem } = useBulkSelect(pasajeroIDs);
    const [allPasajeros] = useState(pasajeros.data.slice(0,21));
    const [primarypasajeros, setPrimaryPasajeros] = useState(allPasajeros);
    ...
    const {
        ...
        prevPage,
        goToPage
    } = usePagination(primarypasajeros, 7);
...
export default ViewPasajerosCard;