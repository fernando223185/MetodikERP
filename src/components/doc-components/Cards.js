  ...
  {['Primary', 'Secondary', 'Success', 'Danger', 'Warning', 'Info', 'Light', 'Dark'].map(
    (variant, idx) => (
      <Col sm={6} lg={4} key={variant}>
        ...
        >
          <Card.Body>
            <Card.Title as="div" className={classNames({'text-white' : variant !== 'Light', 'text-black' : variant === 'Light' })}>{variant} Card Title </Card.Title>
            <Card.Text>
...
<Card className="bg-dark text-white" data-bs-theme="light" style={{ maxWidth: '30rem' }}>
...
const Card = () => {
  return (
    <>
      ...
      >
        <Button
          href={`${reactBootstrapDocsUrl}/docs/components/cards`}
          target="_blank"
          variant="link"
          size="sm"
          className="ps-0"
