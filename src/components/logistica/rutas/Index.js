...
import { useGetRuta } from '../../../hooks/Logistica/Ruta/useRutas';
import { useGetFiltroModulo } from '../../../hooks/useFiltros'; 
import { useLocation } from "react-router";
import ViewReservasCard from './sections/ViewRutasCard'
...
const Rutas = () => {
  ...
  const { getRuta, ruta, isLoading: isLoadingRutas } = useGetRuta();
  ...
  if (isLoadingIndicadores || isLoadingRutas) {
    return (
      ...
      <Row className="g-3 mb-3">
        <Col lg={12}>
         {/* <Card>
          ...
          {formview === 'view-card' ? (
            <ViewReservasCard reservas={ruta} movimientos={movimientos} estatus={estatus} layout={formview} setFilter={setFilter} situaciones={situaciones} usuarios={usuarios} />
          ) : (
            <TableRutas ruta={ruta} movimientos={movimientos} estatus={estatus} layout={formview} setFilter={setFilter} situaciones={situaciones} usuarios={usuarios} filter={filter}/>
          )}
        </Col>
      </Row>
    </>
