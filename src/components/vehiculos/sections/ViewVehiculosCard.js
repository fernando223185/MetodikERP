...
import AllVehiculosHeader from '../tables/AllVehiculosHeader';
import VehiculosFilterForm from './VehiculosFilterForm';
import { VehiculosCardLayout } from './VehiculosCardLayout';

const ViewVehiculosCard = ({ vehiculos, estatus, layout, setFilter, tipovehiculo}) => {
    ...
    const vehiculosIDs = vehiculos.data.map(vehiculo => vehiculo.ID);
    const { selectedItems, isSelectedItem, toggleSelectedItem } = useBulkSelect(vehiculosIDs);
    const [allVehiculos] = useState(vehiculos.data.slice(0, 21));
    const [primaryVehiculos, setPrimaryVehiculos] = useState(allVehiculos);
    ...
    const {
        ...
        prevPage,
        goToPage
    } = usePagination(primaryVehiculos, 7);
...
export default ViewVehiculosCard;