...
export const VehiculosCardLayout = ({data, isSelectedItem, toggleSelectedItem}) => {
    console.log("esto es la data de layout", data);

    return (
    <div className='d-flex flex-column gap-3'>
            {data.slice(0,12).map((vehiculo,index)=> (
                <div 
                key={index}
                ...
                >
                  <div className="d-flex align-items-start align-items-sm-center">
                    ...
                    >
                      <Form.Check.Input
                        type="checkbox"
                        checked={isSelectedItem(vehiculo.id)}
                        onChange={() => toggleSelectedItem(vehiculo.id)}
                      />
                    </Form.Check>
                    <Link
                        to={`/Catalogos/view-vehiculos/${vehiculo.id}`}
                        className="d-none d-sm-block"
                    >
                      <Avatar size="3xl" name={vehiculo.avatar.name} />
                    </Link>
                    <div className='ms-1 ms-sm-3'>
                        <p className='fw-semi-bold mb-3 mb-sm-2'>
                            <Link to={`/Catalogos/view-vehiculos/${vehiculo.id}`}>{vehiculo.Vehiculo}</Link>
                        </p>
                        <Row className="align-items-center gx-0 gy-2">
                            ...
                            <Col xs="auto">
                                <h6 className='mb-0 text-500'>{vehiculo.FechaEmision}</h6>
                            </Col>
                        </Row>
                    </div>
                </div>
