...
export const ModulosCardLayout = ({ data, isSelectedItem, toggleSelectedItem}) => {

    return (
        <div className='d-flex flex-column gap-3'>
            {data.slice(0,12).map((modulo,index)=> (
                <div 
                key={index}
                ...
                >
                    ...
                    >
                        <Form.Check.Input
                        type="checkbox"
                        checked={isSelectedItem(modulo.id)}
                        onChange={() => toggleSelectedItem(modulo.id)}
                        />
                    </Form.Check>
                    <Link
                        to={`/configuracion/modulos/${modulo.id}`}
                        className="d-none d-sm-block"
                    >
                        <Avatar size="3xl" name={modulo.avatar.name} />
                    </Link>
                    <div className='ms-1 ms-sm-3'>
                        <p className='fw-semi-bold mb-3 mb-sm-2'>
                            <Link to={`/configuracion/modulos/${modulo.id}`}>{modulo.avatar.name} {modulo.Tipo}</Link>
                        </p>
                        <Row className="align-items-center gx-0 gy-2">
                            ...
                            <Col xs="auto">
                                <h6 className='mb-0 text-500'>{modulo.FechaEmision}</h6>
                            </Col>
                        </Row>
                    </div>
                </div>
