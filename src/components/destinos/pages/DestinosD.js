...
import { useGetDestinoID } from "hooks/Catalogos/Destinos/useDestino";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {  faReply } from "@fortawesome/free-solid-svg-icons";
import IconButton from "components/common/IconButton";
import { useGetFiltroCatalogo } from "hooks/useFiltros";
import FormDestinosD from "../form/FormDestinosD";

const DestinosDHeader = () => {
    return (
      <Container fluid className="py-3 px-4 border-bottom mb-4">
        <Row className="align-items-center">
          <Col md={8}>
            <h2 className="mb-0">Configuración de Destinos</h2>
            <span className="text-muted">Administra los Destinos</span>
          </Col>
...
const DestinosD = () => {
    ...
    useEffect(() => {
        const fetchDestinoID = async () => {
            if (id!= null && id>0) {
                await getDestinoID({id})
            }
        };
        fetchDestinoID();
